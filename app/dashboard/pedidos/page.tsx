            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((p) => (
                <tr key={p.id} className="hover:bg-blue-50/50 transition">
                  ...
                  <td className="px-4 py-3.5 text-slate-700 whitespace-nowrap">{p.proveedor}</td>
                  ...
                  <td className="px-4 py-3.5 text-slate-700 font-medium whitespace-nowrap">S/ {p.importe.toFixed(2)}</td>
                  <td className="px-4 py-3.5 whitespace-nowrap">
                    <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-xs font-medium ${estadoStyle[p.estado] ?? ""}`}>
                      <i className={`${estadoIcon[p.estado]} text-[10px]`} />{p.estado}
                    </span>
                  </td>
