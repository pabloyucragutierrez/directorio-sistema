...
export default function NuevoProveedorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (get("codigoPostal"))
        formData.append("codigoPostal", get("codigoPostal"));
      ...
      if (get("representante"))
        formData.append("representante", get("representante"));
      if (get("dni")) formData.append("dni", get("dni"));
      if (get("telefonoRep"))
        formData.append("telefonoRep", get("telefonoRep"));
      const comentarios = get("comentarios").trim();
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Datos para pago" className="sm:col-span-2">
              <textarea
                name="datosPago"
                rows={2}
                placeholder="Banco, cuenta, CCI, número Yape..."
...
const inputCls =
  "w-full bg-white border border-slate-200 rounded-lg px-3.5 py-2 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition";
