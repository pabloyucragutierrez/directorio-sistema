...
export default function ProveedoresPage() {
  ...
  const fetchFirstPage = useCallback(async (q?: string) => {
  ...
  const fetchNextPage = useCallback(async () => {
    ...
    try {
      ...
      setError("Error al cargar más proveedores");
    } finally {
                ...
                proveedores.map((p) => (
                  <tr key={p.id} className="hover:bg-blue-50/50 transition">
                    ...
                    <td className="px-4 py-3.5 whitespace-nowrap">
                      <span
                        className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-xs font-medium ${p.activo ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                      >
                    ...
                    <td className="px-4 py-3.5 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-3">
                        ...
                        >
                          {deletingId === p.id ? <i className="fa-solid fa-spinner fa-spin text-sm" /> : <i className="fa-solid fa-trash text-sm" />}
                        </button>
                      </div>
                    </td>
                  </tr>
