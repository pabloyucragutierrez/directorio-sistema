...
import { api, setProveedorToken, setCurrentProveedor } from "@/lib/api";
...
export default function ProveedorLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(false);
    try {
      const data = await api.loginProveedor(usuarioAcceso, passwordAcceso);
    ...
    <div className="min-h-screen bg-white flex flex-col lg:flex-row">
      <div className="hidden lg:flex lg:w-1/2 bg-blue-700 flex-col items-center justify-center p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          ...
          <h2 className="text-3xl font-semibold text-white mb-3">Portal de Proveedores</h2>
          <p className="text-blue-200 text-sm leading-relaxed max-w-xs">
            Revisa y responde los pedidos que te han enviado desde un solo lugar.
          </p>
          <div className="mt-10 space-y-3 text-left">
            {["Recibe pedidos en tiempo real", "Acepta o rechaza con un clic", "Accede desde cualquier dispositivo"].map((item) => (
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-10">
        <div className="w-full max-w-sm">
          <div className="lg:hidden flex flex-col items-center mb-8 gap-3">
            ...
            <span className="text-base font-semibold text-slate-700">Portal de Proveedores</span>
          </div>

          <div className="mb-8">
            <h1 className="text-2xl font-semibold text-slate-800 tracking-tight">Acceso proveedor</h1>
            <p className="text-sm text-slate-500 mt-1">Ingresa tus credenciales para ver tus pedidos</p>
          </div>
          ...
          {error && (
            <div className="mb-4 px-4 py-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-600 flex items-center gap-2">
              ...
              <span>Usuario o contraseña incorrectos. Vuelve a intentarlo.</span>
            </div>
          )}

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1.5 uppercase tracking-wider">Usuario</label>
              <div className="relative">
                <i className="fa-solid fa-user absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-sm" />
                <input
                  type="text" value={usuarioAcceso} onChange={(e) => setUsuarioAcceso(e.target.value)}
